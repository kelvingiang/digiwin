    ...
    <div class="container mx-auto">
        <div class="count-grid">
            ...
            <div class="count-item">
                <a href="<?php echo esc_url(home_url('/about-' . dgw_get_lang())); ?>">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // [2026-07-08] Tối ưu hóa để hiệu ứng mượt hơn, sử dụng requestAnimationFrame và easing function
        const runCounter = (counter) => {
            const target = +counter.getAttribute('data-target');
            const duration = 2000; // Tổng thời gian chạy số (miligiây)
            let startTime = null;
            ...
            const step = (currentTime) => {
                ...
                // Áp dụng hiệu ứng ease-out để số chạy chậm dần về cuối, tạo cảm giác mượt hơn
                const easeOutProgress = 1 - Math.pow(1 - progress, 3);
                const currentNumber = Math.floor(easeOutProgress * target);

                counter.innerText = currentNumber.toLocaleString();

                if (progress < 1) {
                    // Sử dụng requestAnimationFrame thay cho setTimeout để animation khớp với tần số làm tươi của màn hình
                    requestAnimationFrame(step);
                } else {
                    // Đảm bảo số cuối cùng chính xác và có định dạng
